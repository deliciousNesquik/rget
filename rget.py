...
class RemoteFileSyncClient:
        ...
        if self.accept_new:
            ...
            if known_hosts_path.exists():
                ...
                try:
                    ...
                    self.logger.debug(
                        f"Connected with verified host key from {known_hosts_path}"
                    )
                    return conn
                    ...
                    if (
                        "is not in the known_hosts file" not in error_msg
                        and "host key is not trusted" not in error_msg
                    ):
                        # Ключ изменился — это MITM!
                        if (
                            "does not match" in error_msg
                            or "host key has changed" in error_msg
                        ):
            ...
            self.logger.warning(
                f"New host detected: {self.host}:{self.port}. Accepting key..."
            )
            ...
            if known_hosts_path.exists():
                connection_kwargs["known_hosts"] = str(known_hosts_path)
                self.logger.debug(
                    f"Using strict host key verification with {known_hosts_path}"
                )
            else:
                self.logger.error(
                ...
                raise RuntimeError(
                    "Host key verification impossible: known_hosts missing"
                )
    ...
    async def download_file(
        self, sftp: asyncssh.SFTPClient, remote_path: str, local_path: Path
    ) -> tuple[str, bool]:
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="A high-performance utility for downloading files from remote servers.",
...
For quick setup, edit the Config class in the script.
    ...
    parser.add_argument("local_dir", help="Local directory for saving files")
    ...
    auth_group.add_argument("--passphrase", help="Passphrase for SSH key (if required)")
    ...
    perf_group.add_argument(
        ...
        type=int,
        default=(
            Config.DEFAULT_MAX_CONCURRENT if Config.DEFAULT_MAX_CONCURRENT else "None"
        ),
    ...
    # Other options
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Detailed output for debugging"
    )

    parser.add_argument("--version", action="version", version=f"rget v{VERSION}")
...
async def async_main():
    ...
    try:
        ...
        try:
            # Find files
            remote_files = await client.find_files(
                conn_search, args.remote_dir, args.pattern
            )

            if not remote_files:
                logger.error(
                    f"Files by pattern '{args.pattern}' not found in {args.remote_dir}"
                )
                return 1
        ...
        if args.dry_run:
            if not args.verbose:
                ...
                logger.info(
                    f"Total: {len(remote_files)} file(s) | Use without --dry-run to download"
                )
        ...
        tasks = [
            client.download_files_sequential(
                group, local_dir, download_conns[i], master_pbar
            )
        ...
        # Print results
        print(
            f"Successfully downloaded: {len(all_success)}/{len(remote_files)} file(s)"
        )
        ...
        if args.verbose:
            ...
            if all_success:
                logger.debug(
                    f"Average time per file: {duration / len(all_success):.2f} seconds"
                )
        ...
        try:
            ...
            pass  # Если использовали --dry-run соединения не создавались.
